...
library(dplyr)
cokriging_lmc_mle <- function(dat,variables, fix.nu = FALSE, fix.scale = !fix.nu,
                              ...
                              K,seed = 2043)
{
  ...
  # LMC  model structure with two components and nugget effect 
  ...
  for(k in 1:K)
  {
    ...
    train_dat[,4:5] <- scale(train_dat[,4:5])   ## scale the target variables, since here assume
    ...
    resk <- RFfit(pars.model, x= train_dat[indexk,"longitude"], y = train_dat[indexk,"latitude"], 
                  data = train_dat[indexk,4:5],split = FALSE,lower = lower, upper = upper)
    ...
    test_error <- test_error + colSums((test_dat[,4:5] - pred)^2,na.rm = TRUE)/num_obs
  }
