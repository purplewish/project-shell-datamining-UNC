                            ...
                            K,seed = 2043)
{
  ...
  for(k in 1:K)
  {
    ...
    pred <- fitpred_RF(train_dat, test_dat, loc_variables = loc_variables, variables = variables,
                       nu1 = nu1, nu2 = nu2, index_log = index_log)
...
# output contains two sets of 
tunning_RF <- function(dat, loc_variables, variables, numat, index_log, K ,seed)
{
  ...
  index_min <- which.min(rowSums(sweep(res_mat, 2, mean_vec,FUN = "/"))) # relative min
...
cv_RF <- function(dat, loc_variables, variables, numat, index_log, K ,seed)
{
  ...
  for(k in 1:K)
  {
    ...
    nu_select <- tunning_RF(train_dat, loc_variables = loc_variables, variables = variables, 
	                numat = numat, index_log = index_log, K = K ,seed = seed)
    
    pred <- fitpred_RF(train_dat, test_dat, loc_variables = loc_variables, variables = variables, nu1 = as.numeric(nu_select[1]), nu2 = as.numeric(nu_select[2]), index_log = index_log)
