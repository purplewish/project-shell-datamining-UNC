#################### local polynomial with second order ############################
...
# loc_variables: specify the location variable names
#varname: the names of the interpolated variables
# span0: the value of the span
# K : number of fold, default value is K = 10
# seed: random seed for cross validation, default value is 2043
#############################################################
library(cvTools)
library(dplyr)
fitpred_loess <- function(train_dat, test_dat, loc_variables, varname, span0)
{
  ...
  for(j in 1:nvar)
  {
    ...
    formuj <- as.formula(paste(varname[j],"~",paste(loc_variables,collapse = "+"))) # formula
    resj <- loess(formuj, data = train_dat[indexj,], span = span0[varname[j]], control=loess.control(surface = "direct")) # model fitting
    predj <- predict(resj,newdata = test_dat[,loc_variables]) # prediction
    predmat[,j] <- predj
  }
  return(predmat)
}
