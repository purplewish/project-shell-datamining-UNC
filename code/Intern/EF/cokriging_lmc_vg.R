...
# tunning_lmc_vg gives the selected tunning parameter
# cv_lmc_vg gives the test RMSE based on selected tunning parameter 
                   ...
                   K,seed = 2043)
{
  ...
  index_min <- which.min(rowSums(sweep(res_lmc_vg, 2, mean_vec,FUN = "/"))) # relative min
  param_select <- param[index_min,]
  return(param_select)
}

# kapppa and range 
cv_lmc_vg <- function(dat, loc_variables, variables, param, index_log, K ,seed)
{
  set.seed(seed)
  nv <- length(variables)
  cv_group <- cvFolds(n = nrow(dat),K = K,type = "random")
  ...
  for(k in 1:K)
  {
    ...
    param_select <- tunning_lmc_vg(train_dat, loc_variables = loc_variables, variables = variables, param = param, 
	               index_log = index_log, K = K ,seed = seed)
    
    pred <- fitpred_lmc_vg(train_dat, test_dat, loc_variables = loc_variables, variables = variables, kappa = as.numeric(param_select[1]),
                     	range = as.numeric(param_select[2]), index_log = index_log)
